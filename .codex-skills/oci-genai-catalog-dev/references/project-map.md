...
- `scripts/export-native-model-matrix.sh`: local OCI CLI exporter for the complete native model offering across active subscribed regions, including all returned lifecycle, capability, and dedicated-shape fields; it can also produce a CLI-only `models.json`-shaped comparison snapshot.
...
- Export native model and dedicated-shape data from an authenticated OCI CLI profile:
  - `scripts/export-native-model-matrix.sh --compartment-id 'YOUR_COMPARTMENT_OCID' --output genai-offering-cli.json --catalog-output models-cli.json`
